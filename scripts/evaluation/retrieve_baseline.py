...
def main():
    # 1. 加载资源
    logger.info(f"正在连接数据库: {settings.MILVUS_DB_PATH}")
    client = MilvusClient(str(settings.MILVUS_DB_PATH))

    logger.info(f"正在加载模型: {settings.MODEL_PATHS['embedding']}")
    ...
    if not os.path.exists(test_data_path):
        logger.error(f"错误: 找不到测试集文件 {test_data_path}")
        return
    ...
    for i, hits in enumerate(res):
        ...
        if target_id not in retrieved_ids[:10]:
            logger.warning("Missed Case:")
            # print(f"Query: {query}")
            # print(f"Truth ID: {ground_truth_id}")
            # 打印排在前面的几个错误答案
            logger.debug(f"Top-1 Wrong Result: {res[i][0]['entity']['text'][:50]}...")


    # 5. 输出结果
    logger.info("="*30)
    logger.info(f"基线评估结果 (Total: {total})")
    logger.info("="*30)
    logger.info(f"Recall@5 : {hits_at_5/total:.2%}")
    logger.info(f"Recall@10: {hits_at_10/total:.2%}")
    logger.info("="*30)
